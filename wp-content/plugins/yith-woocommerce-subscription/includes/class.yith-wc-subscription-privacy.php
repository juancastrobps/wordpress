			...
			if ( $subscriptions ) {
				foreach ( $subscriptions as $subscription ) {
					if ( apply_filters( 'ywsbs_privacy_erase_personal_data', $erasure_enabled, $subscription ) ) {
						...
						$response['messages'][]    = sprintf( __( 'Removed personal data from subscription %s.', 'woocommerce' ), $subscription->ID );
						$response['items_removed'] = true;
					} else {
						/* Translators: %s Order number. */
						$response['messages'][]     = sprintf( __( 'Personal data within subscription %s has been retained.', 'woocommerce' ), $subscription->ID );

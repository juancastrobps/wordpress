...
if ( ! function_exists( 'charitize_activity_array' ) ) :
    ...
    //codigo agregado: La sección de actividades se basa en los productos que se refieren a las campañas en este caso.
        $charitize_products_args =    array(
            'post_type' => 'product'
        );
            
        if( !empty( $charitize_activities_args )){
            // the query
            $charitize_products = new WP_Query ($charitize_products_args);
            //var_dump($charitize_products);
            if ( $charitize_products->have_posts() ) :
                $i = 0;
                while ( $charitize_products->have_posts() ) : $charitize_products->the_post();
                    $url ='';
                    if(has_post_thumbnail()){
                        $thumb = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'charitize-home-activities-image' );
                        $url = $thumb['0'];
                    ...
                    $charitize_activities_contents_array[$i]['charitize-activities-link'] = get_permalink();
                    $charitize_activities_contents_array[$i]['charitize-activities-image'] = $url;
                    // $charitize_activities_contents_array[$i]['charitize-activities-percent'] = get_raised_percent();
                    //var_dump(get_fund_raised_percent_format());
                    $i++;
                endwhile;
                wp_reset_postdata();
            endif;    
